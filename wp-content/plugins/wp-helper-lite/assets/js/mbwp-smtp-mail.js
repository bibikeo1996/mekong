jQuery( document ).ready(function($) {
    ...
    $("input[data-depend-id=mbwp-smtp-setting]").on("click",function (){
        let value = $(this).val();
        let smtp_host = $("input[data-depend-id=mbwp-smtp-host]");
        let smtp_port = $("input[data-depend-id=mbwp-smtp-port]");
        let smtp_security = $("input[data-depend-id=mbwp-smtp-security]");
    ...
    $("input[data-depend-id=mbwp-smtp-security]").on('change',function (){
        let value = $(this).val();
        let smtp_port = $("input[data-depend-id=mbwp-smtp-port]");
        if(value === 'ssl'){
            smtp_port.val('465');
